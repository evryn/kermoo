...
func TestLoadUserConfig(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			filename: root + "/tests/stubs/valid.json",
			isError:  false,
		},
		{
			name:     "valid yaml file",
			filename: root + "/tests/stubs/valid.yaml",
			isError:  false,
		},
		{
			name:     "invalid json file",
			filename: root + "/tests/stubs/invalid.json",
			isError:  true,
			errMsg:   "unable to unmarshal json content",
		},
		{
			name:     "invalid yaml file",
			filename: root + "/tests/stubs/invalid.yaml",
			isError:  true,
			errMsg:   "invalid yaml configuration",
		},
		{
			name:     "non-existent file",
			filename: root + "/tests/stubs/non_existent.json",
			isError:  true,
			errMsg:   "unable to read file",
		},
		{
			name:     "valid json but does not match user_config_type",
			filename: root + "/tests/stubs/invalid_structure.json",
			isError:  true,
			errMsg:   "schema version is not supported",
		},
		{
			name:     "valid yaml but does not match user_config_type",
			filename: root + "/tests/stubs/invalid_structure.yaml",
			isError:  true,
			errMsg:   "schema version is not supported",
		},
		{
